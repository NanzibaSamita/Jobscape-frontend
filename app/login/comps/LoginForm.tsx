...
export default function LoginForm() {
    ...
    const handleLogin = async (data: any, token: string) => {
        ...
        await loginAction(userData.id, userData.user_name, token, roleId, roleWeight);
    ...
    return (
        <>
            <div className="w-full">
                {loading && <div className="absolute top-0 left-0 h-full w-full flex items-center justify-center backdrop-blur-sm z-[2]">
                ...
                <div className="max-w-sm w-full mx-auto translate-x-0 lg:translate-x-16 px-2 py-6 lg:px-0 lg:py-0 max-h-screen flex flex-col lg:mt-16 mt-4">
                    ...
                    <div className="lg:mt-24 mt-24">
                        <div className="">
                            <div className="mb-4">
                                <h2 className="text-2xl lg:text-3xl font-bold mb-2">Welcome Back <br /> Wanted.Ai!</h2></div>
                        </div>
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit(handelContinue)} className="space-y-2">
                                <FormField
                                ...
                                <div className='space-y-5'>
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem className="flex items-center space-x-2">
                                                    ...
                                                    <FormLabel htmlFor="rememberMe" className="text-xs font-extralight cursor-pointer">
                                                        Remember me
                                                    </FormLabel>
                                                </FormItem>
                                            )}
                                    ...
                                    <BlackStyleButton fullWidth disabled={loading} title={loading ? <Loader2 className="animate-spin" /> : "Sign in"} />
                                </div>
                            </form>
                        </Form>
                        <p className='text-sm font-light text-center py-2'>Don&apos;t have an account yet? <span className='cursor-pointer text-yellow-600' onClick={() => router.push("/signup")}>Sign up</span></p>
                    </div>
                </div>
            </div>
