...
export default function EmployerInboxPage() {
  ...
  // If redirected with an application ID, we can optionally use it to highlight a room, but for now we fetch all
  ...
  // Auto-select room if application query param is present
  useEffect(() => {
    if (rooms.length > 0 && !selectedRoom && applicationId) {
      const room = rooms.find(r => r.application_id === applicationId);
      if (room) {
        setSelectedRoom(room);
      }
    }
  }, [rooms, applicationId, selectedRoom]);
